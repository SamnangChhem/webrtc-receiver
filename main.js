...
answerButton.onclick = async () => {
  const sharedCallIdDoc = firestore.collection('shared').doc('callId');

  try {
    // Retrieve the call ID from Firestore
    const doc = await sharedCallIdDoc.get();
    if (!doc.exists) {
      console.log('No such document!');
      return;
    }

    const callId = doc.data().id;
    ...
    // Enable hangup button once connected
    hangupButton.disabled = false;
